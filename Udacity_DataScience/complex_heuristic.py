...
def complex_heuristic(file_path):
    ...
    for passenger_index, passenger in df.iterrows():
        passenger_id = passenger['PassengerId']
        if passenger['Survived'] == 1 and passenger['Sex'] == 'male' and passenger['Age'] > 18 and passenger['Pclass'] > 1:
            new_df = DataFrame({'Parch':Series(passenger['Parch']),
                                'SibSp': Series(passenger['SibSp']),
                                'Age':Series(passenger['Age']),
                                'Embarked': Series(passenger['Embarked']),
                                'PassengerId':Series(passenger['PassengerId']),
                                'Sex':Series(passenger['Sex']),
                                'Survived':Series(passenger['Survived'])
                                })
            print new_df
    print ''
    for passenger_index, passenger in df.iterrows():
        passenger_id = passenger['PassengerId']
        if passenger['Survived'] == 0 and passenger['Sex'] == 'female':
            surprise = DataFrame({'Parch':Series(passenger['Parch']),
                                'SibSp': Series(passenger['SibSp']),
                                'Age':Series(passenger['Age']),
                                'Embarked': Series(passenger['Embarked']),
                                'PassengerId':Series(passenger['PassengerId']),
    ...
    #print df[['Survived','Fare','Sex']]

    return predictions

complex_heuristic(r'C:\Users\CJH\PycharmProjects\Udacity_DataScience\titanic-data.csv')