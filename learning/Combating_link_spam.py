...
def is_reciprocal_link(graph,source,destination,k):
    if k == 0:
        if source == destination:
            return True
        return False
    if source in graph[destination]: # This means from destination we can link to source,so it makes up a reciprocal link
        return True
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    ### This is to judge whether this link is reciprocal
...
#graph ={'a': ['a', 'b', 'c'], 'c': ['d'], 'b': ['a'], 'e': ['b'], 'd': ['a', 'e']}

# For example

graph = {'a': ['a', 'b', 'c'], 'b':['a'], 'c':['d'], 'd':['a']}
print compute_ranks(graph, 0) # the a->a link is reciprocal
#>>> {'a': 0.26676872354238684, 'c': 0.1216391112164609,
#     'b': 0.1216391112164609, 'd': 0.1476647842238683}

print compute_ranks(graph, 1) # a->a, a->b, b->a links are reciprocal
