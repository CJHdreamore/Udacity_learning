...
def ordered_search(index, ranks, keyword):
    ...
    if urls:
        ...
        for entry in urls:
            if entry in ranks:
                # take out the value
                value_list.append(ranks[entry])
                pickout[entry] = ranks[entry]

        ranked_value = quicksort(value_list)


        ranked_page = []
        i = 0
        while i < len(ranked_value):
            for score in pickout:
                if ranked_value[i] == pickout[score]:
                    ranked_page.append(score)
            i = i + 1
        return ranked_page
    return None
...
def quicksort(list):
    ...
    for i in range(1, n):
        if pivot >= list[i]:
            lower_list.append(list[i])

        else:
            higher_list.append(list[i])
    sorted_list = quicksort(higher_list) + [pivot] + quicksort(lower_list)
    return sorted_list
